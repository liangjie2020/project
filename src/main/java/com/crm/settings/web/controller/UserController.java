    ...
    private void login(HttpServletRequest request, HttpServletResponse response) {
        ...
        try{
            ...
            */
            ...
            e.printStackTrace();
            //一旦程序执行了catch块的信息，说明业务层验证登录失败，为controller抛出了异常
            //表示登录失败{"success":false,"msg":?}
            ...
            * 可以有两种手段来处理
            * （1）将多项消息打包成为map,将map解析为json串
            * （2）创建一个Vo
            *     private boolean success;
            *     private String msg;
            * 如果对于展现的信息将来还会大量的使用，会创建一个Vo类使用方便
            * 如果对于展现的信息只有在这个需求中能够使用，使用map即可
            * */
